...
async function getAllEmbeddingForAcquiredLessonKnowledgePoints(knps: string[]) {
    // return getEmbedding(knowledge);
    return knps.map(knp => getEmbedding(knp));
}
export async function POST(req: NextRequest, res: NextResponse) {
    ...
    try {
        ...
        const { id, status }: z.infer<typeof createLessonSchema> = parseResult.data;
        if (!id || !status) return NextResponse.json({ error: "id or status not found in request @POST" }, { status: 400 });
        const lesson = await getLesson(id);
        if (!lesson) return NextResponse.json({ error: "Lesson not found with given lesson ID @POST" }, { status: 404 });
        //the cool stuff
        const { knowledgePointsFromLesson } = lesson;
        const embeddings = await Promise.all(await getAllEmbeddingForAcquiredLessonKnowledgePoints(knowledgePointsFromLesson));
        const knowledgePointsTransaction = await prisma.$transaction(async (tx) => {
            //add the knowledgePoints to the user

...
function LessonItem({ topic, currentInformationIntakeSummary, threadID, imageURL }: { topic: string, currentInformationIntakeSummary: string, threadID: string, imageURL: string }) {
    ...
    return (
        <Link href={`/learn/${threadID}`}>
            <div className="flex flex-row gap-0 w-[32rem] h-56" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} style={{
                ...
                <article className='flex flex-col justify-between h-full' style={{ padding: spacing.padding.normalY, rowGap: spacing.gaps.groupedElement }}>
                    <div className="flex flex-col gap-2">
                        ...
                        <h3 className='max-h-full text-ellipsis'>{currentInformationIntakeSummary}</h3>
                    </div>
                    <div className="flex gap-2 items-center">
                ...
                <Image src={imageURL} alt="Lesson Image" width={200} height={200} />

...
function SwitcherButton(props: { text: string, setMode: (mode: number) => void, mode: number }) {
    ...
    return (
        <button style={{ height: 2 * spacing.gaps.separateElement - 10 }} className="flex flex-col items-center justify-between relative px-3 switcherButton transition-all" onClick={() => setMode(thisMode)}>
            <Link
                href="#"
                style={{ color: mode == thisMode ? 'black' : '' }} className={` font-bold text-muted-foreground transition-all hover:text-foreground`}
            >
                {text}
            </Link>
            {

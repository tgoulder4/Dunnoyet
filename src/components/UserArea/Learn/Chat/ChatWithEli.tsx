...
function ChatWithEli({
    ...
    async function submitUserQuestion(data: FormData) {
        ...
        const response: any = await new Promise<{ newStatePayload: IMessagesEndpointResponsePayload } | {
            error: string
        }>((resolve, reject) => {
            setTimeout(() => {
                resolve({
                    newStatePayload: {
                        newMessages: [
                            {
                                content: '\n' +
                                    "As you know about wave-particle duality, we'll talk about how that relates to understanding electron behavior as waves.",
                                role: 'eli',
                ...
                } as { newStatePayload: IMessagesEndpointResponsePayload })
            }, 1000);
        })
        ...
        console.log("newStatePayload: ", response.newStatePayload)
        const { newStatePayload } = response as { newStatePayload: IMessagesEndpointResponsePayload }
        ...
        console.log("Updating state with newStatePayload");
        await updateState(data, newStatePayload);
        setType('Lesson');
    }
    return (<div style={{ right: sizing.variableWholePagePadding }} className='flex flex-col bottom-0  z-10 w-full max-w-[600px] fixed rounded-t-[10px] shadow-[0px_0px_0px_2px_#131313]'>
        ...
        <form action={_type == "Lesson" ? updateState : _type == "NewQ" ? submitUserQuestion : _type == "Tutorial" ? "" : ""}>
            {
                isOpen ? <>
                    <div className='h-[60vh] w-full flex flex-col items-center' style={{ backgroundColor: type == 'Tutorial' ? changeColour(colours.primary).darken(8).toString() : colours.complementary_lightest, paddingTop: 0 }}>
                        {_type == "NewQ" && <>
                            <div className="w-full px-8 pt-9 flex items-center justify-center">
                                <Message className='w-full h-48 p-[30px]' style={{ marginBottom: spacing.gaps.groupedElement, borderBottomRightRadius: 0, backgroundColor: changeColour(colours.primary).darken(8).toString() }}>
                                    <textarea placeholder='Ask a question...' style={{ fontFamily: merriweather.style.fontFamily, fontSize: sizing.largerFontSize + 'rem', fontWeight: 800 }} id="userInput" name="userInput" className='resize-none text-start bg-transparent border-opacity-5 border-black !placeholder-[#0F291B] text-white flex w-full rounded-md border-2 border-input px-3 py-2 ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#000000] disabled:cursor-not-allowed disabled:opacity-50'></textarea>
                                </Message>
                            </div>
                        </>}
                        {_type == "Lesson" && <Conversation messages={lessonState?.messages!} />}
                    ...
                    <div style={{ columnGap: spacing.gaps.groupedElement }} className="p-8 pb-16 flex flex-row h-max bg-white">
                        {
                            type == "Lesson" && <Input id="userInput" name="userInput" type="text" placeholder="Type your reply..." className='w-full flex-1' />
                        }

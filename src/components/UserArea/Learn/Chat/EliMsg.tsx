...
function EliMessage({ eliResponseType, splitResponse, text }: { text?: string, eliResponseType: "General" | "WhatComesToMind" | "ChallengeQ" | 'SubjectIntroduction', splitResponse?: { text: string, active: boolean } }) {
    return (
        <div className="w-full flex items-start justify-start">
            <Message className='w-full md:w-min' style={{ borderBottomLeftRadius: eliResponseType == "SubjectIntroduction" ? 10 : 0, width: eliResponseType == 'SubjectIntroduction' ? '100%' : '2/3', backgroundColor: eliResponseType == "WhatComesToMind" || eliResponseType == 'ChallengeQ' ? colours.interrogativeMessage : eliResponseType == 'SubjectIntroduction' ? colours.systemEventMessage : colours.complementary_lightest }}>

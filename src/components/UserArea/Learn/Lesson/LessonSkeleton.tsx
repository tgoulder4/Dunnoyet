...
function LessonSkeleton({ params, payload, children }: { params: any, payload: any, children: React.ReactNode }) {
    const lessonXPadding: string = 'clamp(24px,4vw,400px)';
    const [messageHistory, setMessageHistory] = useState([]);
    const parsed = messagesPayloadSchema.safeParse(payload);
    console.dir(parsed, { depth: null })
    if (!parsed.success) {
        console.error("Invalid format, failed parse @LSK: ", payload, " error: ", parsed.error)
        parsed.error
        return <></>
    }
    ...
    return (
        <div className="flex h-full">
            ...
            <LessonSection className='flex-[5] learningChatArea' style={{ borderRight: uiBorder(0.1) }}>
                {stage == 'loading' ? <CreatingLesson isFirstTimer={new URLSearchParams(params.params).get('first')} /> : children}
            </LessonSection>

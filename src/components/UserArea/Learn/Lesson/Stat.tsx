...
const Stat: React.FC<statProps> = ({ children, className, loading, statTitle, value, ...props }) => {
    return (
        <div style={{ border: uiBorder(0.2) }} className='w-full p-12 rounded-xl flex flex-col gap-3'>
            {loading ? <div className='flex flex-col gap-3'><div className="animate animate-pulse w-1/2 h-10 rounded-lg bg-gray-200" /><div className='animate animate-pulse rounded-lg w-3/4 h-6 bg-gray-200' /></div> :

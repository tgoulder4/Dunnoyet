...
function LessonItem({ topic, currentInformationIntakeSummary, threadID, imageURL }: { topic: string, currentInformationIntakeSummary: string, threadID: string, imageURL: string }) {
    ...
    return (
        <Link className='flex flex-col-reverse sm:flex-row  gap-0 w-full sm:h-56' href={`/learn/${threadID}`} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} style={{
            borderRadius: '10px',
